  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('public-realtime')
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'settings' }, (payload: any) => {
        setVotingEnabled(payload.new.voting_enabled);
      })
      .subscribe();
  ...
  return (
    <div className="min-h-screen bg-indigo-50 font-sans selection:bg-yellow-300 relative overflow-hidden pb-24">
      ...
      <div className="max-w-xl mx-auto relative z-10 px-4 pt-8 sm:pt-12">
        ...
        <div className="mb-12"><CountdownBanner /></div>

        {!votingEnabled && (
           <div className="mb-8 p-6 bg-red-100 border-2 border-red-200 rounded-3xl text-center text-red-700 font-black animate-bounce shadow-xl">
              ...
              <div className="space-y-6 animate-in fade-in zoom-in duration-300">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                  {CATEGORIES.map(cat => {
                    const Icon = cat.icon;
                    return (
                      <button key={cat.id} disabled={!votingEnabled} onClick={() => { setSelectedCategory(cat); setCurrentView('voting'); setTeacherName(''); setReason(''); }}
                        className={`${cat.color} ${cat.text} p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border-b-8 ${cat.border} shadow-xl hover:shadow-2xl transform hover:-translate-y-2 transition-all active:translate-y-1 active:border-b-0 flex flex-col items-center text-center ${cat.isFeatured ? 'sm:col-span-2' : ''} ${!votingEnabled ? 'opacity-50 grayscale cursor-not-allowed transform-none shadow-none' : ''}`}>
                  ...
                  <form onSubmit={handleSubmitVote} className="space-y-6">
                    <div className="relative group">
                      ...
                      <div className="relative">
                        <input type="text" value={teacherName} onChange={(e) => { setTeacherName(e.target.value); setShowSuggestions(true); }} onFocus={() => setShowSuggestions(true)} onBlur={() => setTimeout(() => setShowSuggestions(false), 200)} placeholder="Ej. Juan Pérez García"
                          className="w-full text-lg px-6 py-4 rounded-2xl bg-indigo-50 border-2 border-indigo-100 focus:border-indigo-400 focus:bg-white outline-none transition-all placeholder:text-indigo-300 font-semibold text-indigo-900 shadow-inner pr-12" required disabled={isSubmitting || !votingEnabled} autoComplete="off" />
                    ...
                    <div>
                      ...
                      <textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Escribe algo chido... Ej. Sus clases son las mejores." rows={4}
                        className="w-full text-lg px-6 py-4 rounded-2xl bg-indigo-50 border-2 border-indigo-100 focus:border-indigo-400 focus:bg-white outline-none transition-all placeholder:text-indigo-300 resize-none font-semibold text-indigo-900 shadow-inner" required disabled={isSubmitting || !votingEnabled} />
                    </div>
                    <button type="submit" disabled={isSubmitting || !votingEnabled} className={`w-full py-5 rounded-2xl font-black text-xl text-white ${selectedCategory.color} ${(isSubmitting || !votingEnabled) ? 'opacity-70 cursor-not-allowed' : 'hover:brightness-110 active:scale-95'} transition-all shadow-lg flex items-center justify-center mt-6`}>
